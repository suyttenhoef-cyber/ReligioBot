...
# Libelles d'affichage pour les matieres du corpus (cf. README, section
# "Schema du corpus"). Toute nouvelle matiere ajoutee s'affiche
# automatiquement avec sa cle brute tant qu'elle n'a pas ete ajoutee ici.
MATIERE_LABELS = {
    "reglementation_fabriques": "Reglementation comptable et legale",
    "usage_logiciel": "Usage du logiciel ReligioSoft",
}

st.set_page_config(page_title="Assistant ReligioSoft", page_icon="⛪", layout="wide")
...
def main():
    st.title("⛪ Assistant ReligioSoft")
    st.caption(
        "Aide sur le logiciel ReligioSoft et sur la reglementation comptable/legale "
        "des fabriques d'eglise en Region wallonne."
    )
    ...
    with st.sidebar:
        ...
        matiere_choice = st.selectbox(
            ...
            help="Restreindre la recherche à une seule matière évite qu'une "
                 "question sur le logiciel ramène des sources réglementaires "
                 "(ou l'inverse) simplement parce qu'elles y ressemblent.",
        )
        ...
        include_historique = st.checkbox(
            ...
            value=False,
            help="À activer uniquement pour traiter un dossier antérieur à "
                 "un changement réglementaire ou une ancienne version du logiciel.",
        )
        ...
        st.divider()
        st.caption(
            "⚠️ Cet assistant est une aide et ne remplace ni une vérification "
            "comptable par un professionnel, ni le helpdesk officiel Vanden "
    ...
    if retriever is None:
        st.error(
            ...
            "`python3 chunk_builder.py corpus_par_matiere/ chunks.jsonl` puis "
            "`python3 embed_chunks.py chunks.jsonl embeddings.npz` pour "
            "générer les embeddings avant de démarrer l'application."
        )
        st.stop()
    ...
    query = st.chat_input(
        "Pose ta question sur ReligioSoft ou la réglementation des fabriques d'église..."
    )
        ...
        with st.chat_message("assistant"):
            ...
            if results:
                ...
                if citation_warnings:
                    st.error(
                        "⚠️ Verification automatique des references : au moins une "
